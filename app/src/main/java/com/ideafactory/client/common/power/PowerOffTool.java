...
public class PowerOffTool {
    ...
    private void putPowerParam(String key, String value) {
        ACache acache = ACache.get(new File(ResourceUpdate.RESOURSE_PATH + ResourceUpdate.PROPERTY_CACHE_PATH));
...
//        OnOffTool.setEnabled((byte) 0, (byte) 1, (byte) 0, (byte) 1);
        ...
        switch (type) {
            ...
            case 3:
                ...
                break;
            case 1:
                ZHBroadControl.getZhBroadControl().rebootMachine();
                break;
            case 4:
                TYTool.sendBroadcast(XBHActions.ACTION_REBOOT);
                break;
            case 5:
                TYTool.sendBroadcast(JYDActions.ACTION_REBOOT);
                break;
        }
    }
    ...
    public void powerShutdown() {
        if (TYTool.boardIsXBH()) {
            TYTool.sendBroadcast(XBHActions.ACTION_SHUTDOWN);
        } else if (TYTool.boardIsJYD()) {
            TYTool.sendBroadcast(JYDActions.ACTION_SHUTDOWN);
        } else {
            Process process = null;
            try {
                process = Runtime.getRuntime().exec("su");
