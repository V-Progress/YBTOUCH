...
public class FixedTextureVideoView extends TextureView
    ...
    private void transformVideo2(int videoWidth, int videoHeight) {
...
//        //第1步:因为默认视频是fitXY的形式显示的,所以首先要缩放还原回来.
        ...
        Log.e(TAG, "getResizedWidth()------------>" + getResizedWidth());
        Log.e(TAG, "videoWidth------------>" + videoWidth);
        Log.e(TAG, "scale------------>" + videoWidth / (float) getResizedWidth());
//
//        //第3步,等比例放大或缩小,直到视频区的一边超过View一边, 另一边与View的另一边相等. 因为超过的部分超出了View的范围,所以是不会显示的,相当于裁剪了.
//        matrix.postScale(1.0f, 1.0f,0, 0);//后两个参数坐标是以整个View的坐标系以参考的
        Log.d(TAG, "transformVideo, maxScale=" + maxScale);


        matrix.postTranslate(getmOffset_X(), getmOffset_Y());
        setTransform(matrix);
        postInvalidate();
        isChangeInit = true;
    ...
    public int getmOffset_Y() {
        return mOffset_Y;
    }

    public void setmOffset_Y(int mOffset_Y) {
        this.mOffset_Y = mOffset_Y;
    }

    public int getmOffset_X() {
        return mOffset_X;
    }

    public void setmOffset_X(int mOffset) {
        this.mOffset_X = mOffset;
    }
    ...
    private boolean isChangeInit = false;
