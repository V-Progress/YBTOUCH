...
public class UnicomServerSide {
    ...
    //单例服务端
    private static UnicomServerSide serverSide;

    private TcpServerThread tcpServerThread = null;

    public  List<Socket> getDevicelist() {
        return devicelist;
    }

    public void setDevicelist(List<Socket> devicelist) {
        UnicomServerSide.devicelist = devicelist;
    }

    private UnicomServerSide() {
    }

    //
    public static UnicomServerSide getInstance() {
        if (serverSide == null) {
            serverSide = new UnicomServerSide();
        }
        return serverSide;
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    for (int i = 0; i < devicelist.size(); i++) {
                        Socket msocket = devicelist.get(i);
                       if (!ipList.contains(msocket.getInetAddress().toString())){
                           ipList.add(msocket.getInetAddress().toString());
                       }
                    }

                    devicelist.add(socket);
                    if (!ipList.contains(socket.getInetAddress().toString())){

                        Log.e(TAG, "添加的socket ip------------>"+ socket.getInetAddress().toString());
                        Log.e(TAG, "ipList------------>"+ipList.size());
                    }else {
                        Log.e(TAG, "没有添加的socket ip------------>"+ socket.getInetAddress().toString());
                    }
    ...
    public OnServicereceivedMsg onSetvicereceivedMsg;
    public void setOnreceivedMsg(OnServicereceivedMsg onreceivedMsg) {
        this.onSetvicereceivedMsg = onreceivedMsg;
    }


    public interface OnServicereceivedMsg {
    ...
    private class ServerThread extends Thread {
        ...
        public ServerThread(Socket socket) {
            this.socket = socket;
            try {
                //将socket的输入流转换为字符流，使用BufferedReader进行读取缓冲
